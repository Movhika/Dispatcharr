...
def _source_relation_model(content_type):
    return {
...
def _playback_source_relation(playback):
    model = _source_relation_model(playback.content_type)
    if model is None or not playback.relation_id:
        return None
    return model.objects.filter(pk=playback.relation_id).first()


def _effective_json_array_match(field, value, relation_alias="relation"):
    ...
    # indexes. Only the boolean existence result needs a NULL fallback.
...
def _effective_json_scalar_match(
    field, value, relation_fallback="NULL", relation_alias="relation"
):
...
def _vod_relation_sql(filters, relation_type):
        ...
        if value:
            ...
            if relation_type == "series":
                sql, sql_params = _effective_json_array_match(
                    field, value, relation_alias="episode_relation"
                )
    ...
    if feature_values:
        ...
        if relation_type == "series":
            ...
            for feature in compatible_features:
                sql, sql_params = _effective_json_array_match(
                    "video_features", feature, relation_alias="episode_relation"
                )
                feature_clauses.append(sql)
                feature_params.extend(sql_params)
    ...
    if resolution:
        ...
        if relation_type == "series":
            sql, sql_params = _effective_json_scalar_match(
                "resolution", resolution, relation_alias="episode_relation"
            )
            episode_conditions.append(sql)
            episode_params.extend(sql_params)
    ...
    if container:
        ...
        if relation_type == "series":
            sql, sql_params = _effective_json_scalar_match(
                ...
                relation_alias="episode_relation",
            )
            episode_conditions.append(sql)
            episode_params.extend(sql_params)

    if relation_type == "series" and technical_conditions:
        # Series-level defaults/relations are cheap to test. Technical data
        # learned from actual episode playback lives on episode relations, so
...
def _selected_relation_queryset(request, relation_type):
    ...
    ).select_related(canonical_field, "m3u_account", "category")
    if not select_all:
...
class VODSourceRelationViewSet(viewsets.ViewSet):
    ...
    @action(
        detail=False,
        methods=["patch"],
        url_path="manual-metadata",
    )
    def manual_metadata(self, request):
        ...
        relation = relation_model.objects.select_related(
            "m3u_account",
            "category" if relation_type != "episode" else "series_relation__category",
        ...
        from .metadata import effective_relation_metadata

        metadata, locked_fields = _validated_manual_source_metadata(
            metadata, locked_fields
        )
        with transaction.atomic():
            relation_model.objects.filter(pk=relation.pk).update(
                manual_metadata=metadata,
                locked_fields=locked_fields,
                updated_at=timezone.now(),
            )
            relation.manual_metadata = metadata
            relation.locked_fields = locked_fields
        ...
        affected_profiles = mark_profile_selections_outdated(
            ...
            policy_ids=profile_ids_using_source_categories(
                [(
                    ...
                    else relation.series_relation.category_id,
                )]
            ),
        )
        return Response(
    ...
    def bulk_manual_metadata(self, request):
        ...
        relation_querysets = [movie_relations, series_relations]
        updated_source_keys = set()
        updated_at = timezone.now()
        ...
        def update_relations(relations):
            ...
            if not relations:
                return
            model = relations[0].__class__
            relation_type = (
                "movie" if model is M3UMovieRelation
                else "series" if model is M3USeriesRelation
                else "episode"
            )
            changed = []
            for relation in relations:
                source_key = (relation_type, relation.pk)
                if source_key in updated_source_keys:
                    continue
                relation.manual_metadata = {
                    **(relation.manual_metadata or {}),
                    **metadata,
                }
                relation.locked_fields = sorted(
                    set(relation.locked_fields or []) | set(metadata)
                )
        ...
        return Response(
            {
                "updated_sources": len(updated_source_keys),
                "updated_titles": 0,
                "profile_update": (
                    "outdated" if affected_profiles else "not_required"
                ),
...
class M3UVODCategoryRelationViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def metadata_defaults(self, request, pk=None):
        ...
        serializer.save()
        from .metadata import sync_category_relation_metadata

        updated_sources = sync_category_relation_metadata(relation)
        from .catalog_cache import bump_catalog_generation
        from .profile_selection import (
            mark_profile_selections_outdated,
            profile_ids_using_source_categories,
    ...
    def bulk_metadata_defaults(self, request):
        ...
        from .metadata import sync_category_relations_metadata

        updated_sources = sync_category_relations_metadata(relations)
        from .catalog_cache import bump_catalog_generation
        from .profile_selection import (
            mark_profile_selections_outdated,
            profile_ids_using_source_categories,
...
class VODListViewSet(viewsets.ModelViewSet):
    """Manage VOD lists and their atomically published item generations."""
    ...
    def get_queryset(self):
        if getattr(self, "swagger_fake_view", False):
            return VODList.objects.none()
        if not _is_admin(self.request.user):
            return VODList.objects.none()
        return VODList.objects.annotate(
            active_item_count=Count(
                "items",
                filter=Q(items__generation=F("active_generation")),
                distinct=True,
            ),
            active_available_item_count=Count(
                "items",
                filter=(
                    Q(items__generation=F("active_generation"))
                    & (
                        Q(items__movie__isnull=False)
                        | Q(items__series__isnull=False)
                    )
                ),
                distinct=True,
            ),
    ...
    def items(self, request, pk=None):
        ...
        if search:
            queryset = queryset.filter(
                Q(movie__display_name__icontains=search)
                | Q(movie__name__icontains=search)
                | Q(series__display_name__icontains=search)
        ...
        availability = str(
            request.query_params.get("availability") or "any"
        ).lower()
        available_query = Q(movie__isnull=False) | Q(series__isnull=False)
        if availability == "available":
            queryset = queryset.filter(available_query)
        elif availability == "unavailable":
            queryset = queryset.exclude(available_query)

        for parameter, lookup in (
            ("year_from", "gte"),
            ("year_to", "lte"),
        ):
            raw_year = str(request.query_params.get(parameter) or "").strip()
            if raw_year:
                if not raw_year.isdigit():
                    raise DRFValidationError({parameter: "Enter a valid year."})
                year = int(raw_year)
                queryset = queryset.filter(
                    Q(**{f"movie__year__{lookup}": year})
                    | Q(**{f"series__year__{lookup}": year})
                    | Q(
                        movie__isnull=True,
                        series__isnull=True,
                        **{f"year__{lookup}": year},
                    )
                )
        ...
        if any(
            ...
            movie_ids = _apply_vod_canonical_filters(
                Movie.objects.all(), filters, "movie"
            ).values("id")
            series_ids = _apply_vod_canonical_filters(
                Series.objects.all(), filters, "series"
            ).values("id")
            queryset = queryset.filter(
                Q(content_type="movie", movie_id__in=movie_ids)
                | Q(content_type="series", series_id__in=series_ids)
            )
        ...
        if any(
            ...
            queryset = queryset.filter(
                Q(
                    ...
                    movie_id__in=movie_relations.values("movie_id"),
                )
                | Q(
                    content_type="movie",
                    include_all_sources=False,
                    source_memberships__movie_relation_id__in=movie_relations.values(
                        "id"
                    ),
                )
                | Q(
                    content_type="series",
                    include_all_sources=True,
                    series_id__in=series_relations.values("series_id"),
                )
                | Q(
                    content_type="series",
                    include_all_sources=False,
                    source_memberships__series_relation_id__in=series_relations.values(
                        "id"
                    ),
                )
            ).distinct()
        ...
        return paginator.get_paginated_response(
            VODListItemSerializer(page, many=True, context={"request": request}).data
        )

    @action(detail=True, methods=["get"], url_path="filter-options")
    def filter_options(self, request, pk=None):
        """Return technical values present in this list's active sources."""
        denied = self._admin_only(request)
        if denied is not None:
            return denied
        vod_list = self.get_object()
        cache_key = (
            f"vod_list_filter_options:{catalog_generation()}:"
            f"{vod_list.pk}:{vod_list.active_generation}"
        )
        cached = safe_cache_get(cache_key)
        if isinstance(cached, dict):
            return Response(cached)

        items = vod_list.items.filter(generation=vod_list.active_generation)
        all_movie_ids = items.filter(
            content_type="movie",
            include_all_sources=True,
            movie__isnull=False,
        ).values("movie_id")
        all_series_ids = items.filter(
            content_type="series",
            include_all_sources=True,
            series__isnull=False,
        ).values("series_id")
        exact_movie_ids = VODListSourceMembership.objects.filter(
            item__list=vod_list,
            item__generation=vod_list.active_generation,
            movie_relation__isnull=False,
        ).values("movie_relation_id")
        exact_series_ids = VODListSourceMembership.objects.filter(
            item__list=vod_list,
            item__generation=vod_list.active_generation,
            series_relation__isnull=False,
        ).values("series_relation_id")
        movie_relations = M3UMovieRelation.objects.filter(
            Q(pk__in=exact_movie_ids) | Q(movie_id__in=all_movie_ids),
            m3u_account__is_active=True,
        ).select_related("m3u_account", "category")
        series_relations = M3USeriesRelation.objects.filter(
            Q(pk__in=exact_series_ids) | Q(series_id__in=all_series_ids),
            m3u_account__is_active=True,
        ).select_related("m3u_account", "category")

        from .policies import enabled_category_map, relation_metadata
        ...
        for relations in (movie_relations, series_relations):
            for relation in relations.iterator(chunk_size=2000):
                ...
                if resolution:
                    result["resolutions"].add(str(resolution).lower())
                container = metadata.get("container_extension")
                if container and str(container).lower() not in IMAGE_VIDEO_CODECS:
                    result["container_extensions"].add(str(container).lower())
        payload = {key: sorted(values) for key, values in result.items()}
        safe_cache_set(cache_key, payload, timeout=3600)
        return Response(payload)

    @action(detail=True, methods=["put"], url_path="manual-items")
    def manual_items(self, request, pk=None):
        """Replace a manual list by publishing one complete new generation."""
        denied = self._admin_only(request)
        if denied is not None:
            return denied
        raw_items = request.data.get("items")
        if not isinstance(raw_items, list):
            raise DRFValidationError({"items": "Expected a list of items."})

        with transaction.atomic():
            vod_list = VODList.objects.select_for_update().get(pk=self.get_object().pk)
            if vod_list.list_type != VODList.ListType.MANUAL:
                raise DRFValidationError({
                    "detail": "Only manual lists can be edited directly."
                })
            next_generation = vod_list.active_generation + 1
            new_items = []
            relation_specs = []
            seen = set()
            for position, raw in enumerate(raw_items):
                if not isinstance(raw, dict):
                    raise DRFValidationError({
                        "items": f"Item {position + 1} must be an object."
                    })
                content_type = str(raw.get("content_type") or "").lower()
                if content_type not in {"movie", "series"}:
                    raise DRFValidationError({
                        "items": f"Item {position + 1} has an invalid content type."
                    })
                if vod_list.content_type not in {
                    VODList.ContentType.ALL,
                    content_type,
                }:
                    raise DRFValidationError({
                        "items": (
                            f"Item {position + 1} is not allowed by this list's "
                            "content type."
                        )
                    })
                try:
                    ...
                    raise DRFValidationError({
                        "items": f"Item {position + 1} needs a canonical_id."
                    })
                key = (content_type, canonical_id)
                if key in seen:
                    raise DRFValidationError({
                        "items": f"Item {position + 1} is duplicated."
                    })
                seen.add(key)
                canonical_model = Movie if content_type == "movie" else Series
                try:
                    canonical = canonical_model.objects.get(pk=canonical_id)
                except canonical_model.DoesNotExist:
                    raise DRFValidationError({
                        "items": f"Item {position + 1} does not exist."
                    })
                relation_ids = raw.get("relation_ids", None)
                if relation_ids is not None and not isinstance(relation_ids, list):
                    raise DRFValidationError({
                        "items": f"Item {position + 1} relation_ids must be a list."
                    })
                if relation_ids == []:
                    raise DRFValidationError({
                        "items": (
                            f"Item {position + 1} must include at least one "
                            "source, or omit relation_ids to include every source."
                        )
                    })
                item = VODListItem(
                    list=vod_list,
                    ...
                    include_all_sources=relation_ids is None,
                    title=canonical.display_name or canonical.name,
                    year=canonical.year,
                    position=int(raw.get("position", position) or 0),
                )
                new_items.append(item)
                relation_specs.append((content_type, canonical, relation_ids))

            VODListItem.objects.bulk_create(new_items)
            memberships = []
            for item, (content_type, canonical, relation_ids) in zip(
                new_items, relation_specs
            ):
                ...
                relation_model = (
                    M3UMovieRelation
                    if content_type == "movie"
                    else M3USeriesRelation
                )
                canonical_field = "movie" if content_type == "movie" else "series"
                relations = list(
                ...
                if len(relations) != len(set(relation_ids)):
                    raise DRFValidationError({
                        "items": (
                            f"One or more sources for {canonical} do not belong "
                            "to that canonical title."
                        )
                    })
                memberships.extend(
                    VODListSourceMembership(
                        item=item,
                        movie_relation=(relation if content_type == "movie" else None),
                        series_relation=(relation if content_type == "series" else None),
                    )
                    for relation in relations
                )
            VODListSourceMembership.objects.bulk_create(memberships)
            vod_list.active_generation = next_generation
            vod_list.sync_status = VODList.SyncStatus.COMPLETE
            vod_list.last_synced_at = timezone.now()
            vod_list.sync_error = ""
            vod_list.save(update_fields=[
                "active_generation", "sync_status", "last_synced_at",
...
class VODPlaybackSessionViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def bulk_metadata(self, request):
        queryset = self._selected_history(request).exclude(relation_id=None)
        selected_count = queryset.count()
        ...
        source_refs = set(
            queryset.order_by().values_list("content_type", "relation_id").distinct()
        )
        updated_count = 0
        movie_ids = set()
        series_ids = set()
        with transaction.atomic():
            for content_type, model in (
                ...
                relation_ids = {
                    relation_id
                    for source_type, relation_id in source_refs
                    if source_type == content_type and relation_id
                }
                relations = list(model.objects.filter(pk__in=relation_ids))
                for relation in relations:
                    manual = dict(relation.manual_metadata or {})
                    locked = set(relation.locked_fields or [])
                    for field, spec in normalized_updates.items():
                        if spec["mode"] == "clear":
                            ...
                            locked.add(field)
                    relation.manual_metadata = manual
                    relation.locked_fields = sorted(locked)
                    if content_type == "movie":
                        movie_ids.add(relation.movie_id)
                    elif content_type == "series":
                        series_ids.add(relation.series_id)
                    else:
                        series_ids.add(relation.episode.series_id)
                if relations:
                    model.objects.bulk_update(
                        relations,
        ...
        if updated_count:
            ...
            if request.data.get("select_all") is not True:
                if (
                    len(movie_ids) + len(series_ids)
                    <= PLAYBACK_METADATA_INLINE_TITLE_LIMIT
                ):
    ...
    def telemetry(self, request, pk=None):
        ...
        playback.save()
        if metadata:
            relation = _playback_source_relation(playback)
            if relation is not None:
                relation.apply_observation(metadata)
            ...
            relation_map.update(
                {
                    ...
                    .select_related(
                        "movie__logo", "m3u_account", "category"
                    )
            ...
            relation_map.update(
                {
                    ...
                    .select_related(
                        "series__logo", "m3u_account", "category"
                    )
