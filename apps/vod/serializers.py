...
class VODPlaybackSessionSerializer(serializers.ModelSerializer):
    ...
    def _detail_target(obj):
        ...
        if obj.content_type == VODSourceAsset.AssetType.MOVIE and obj.source_asset_id:
            relations = list(obj.source_asset.movie_relations.all())
            relation = next(
                (row for row in relations if str(row.pk) == str(obj.relation_id)),
                relations[0] if relations else None,
            )
            if relation is not None:
                target = ("movie", relation.movie_id, relation.id)
        elif obj.content_type == VODSourceAsset.AssetType.SERIES and obj.source_asset_id:
            relations = list(obj.source_asset.series_relations.all())
            relation = next(
                (row for row in relations if str(row.pk) == str(obj.relation_id)),
                relations[0] if relations else None,
            )
        ...
        elif obj.content_type == VODSourceAsset.AssetType.EPISODE:
            target = ("series", None, None)
            relations = (
                list(obj.source_asset.episode_relations.all())
                if obj.source_asset_id
                else []
            )
            relation = next(
